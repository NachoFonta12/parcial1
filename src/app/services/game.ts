...
export class GameService {
    ...
    async getHighScores(game: number, amount: number = 3, ascending: boolean = false) {
        const {data, error} = ((await this.client.from('results').select('users(name), score').eq('game_id', game).order('score', {ascending: ascending}).limit(amount)));
